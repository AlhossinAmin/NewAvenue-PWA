...
const state = reactive<Record<string, unknown>>(
  props.record
    ? {
        ...props.record,
        customer: props.record.customer?.id ?? "",
        assigned_agent: props.record.assigned_agent?.id ?? "",
      }
    : createEmptyState(LEAD_FIELDS),
);
const loading = ref(false);

const { visibleFields, validate } = useResourceForm(LEAD_FIELDS, state);

const submitLabel = computed(() =>
  props.record ? "Save changes" : "Create lead",
);
...
const onSubmit = async (event: FormSubmitEvent<Record<string, unknown>>) => {
  ...
  try {
    if (props.record) {
      await updateLead(props.record.id, event.data as Partial<LeadInput>);
