...
const props = defineProps<{
  // Omit (or null) to create; pass a contact to edit it.
  record?: Contact | null;
}>();
...
const state = reactive<Record<string, unknown>>(
  props.record ? { ...props.record } : createEmptyState(CONTACT_FIELDS),
);
const loading = ref(false);

const { visibleFields, validate } = useResourceForm(CONTACT_FIELDS, state);

const submitLabel = computed(() =>
  props.record ? "Save changes" : "Create contact",
);
...
const onSubmit = async (event: FormSubmitEvent<Record<string, unknown>>) => {
  ...
  try {
    if (props.record) {
      await updateContact(props.record.id, event.data as Partial<ContactInput>);
