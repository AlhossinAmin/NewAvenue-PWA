...
const props = defineProps<{
  // Omit (or null) to create; pass a member to edit it.
  record?: Member | null;
}>();
...
const state = reactive<Record<string, unknown>>(
  props.record ? { ...props.record } : createEmptyState(MEMBER_FIELDS),
);
const loading = ref(false);

const { visibleFields, validate } = useResourceForm(MEMBER_FIELDS, state);

const submitLabel = computed(() =>
  props.record ? "Save changes" : "Create member",
);
...
const onSubmit = async (event: FormSubmitEvent<Record<string, unknown>>) => {
  ...
  try {
    if (props.record) {
      await updateMember(props.record.id, event.data as Partial<MemberInput>);
      toast.add({ title: "Member updated", color: "success" });
    } else {
      await createMember(event.data as MemberInput);
